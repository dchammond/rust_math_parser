...
mod tests {
    ...
    fn addition() {
        ...
        assert_eq!(my_parser.parse(input).unwrap(), (1+2) as f64);
    }
    #[test]
    fn subtraction() {
        let input = String::from("18 - 4");
        let mut my_parser = parser::Parser::new();
        assert_eq!(my_parser.parse(input).unwrap(), (18-4) as f64);
    }
    #[test]
    fn multiplication() {
        let input = String::from("3 * 6");
        let mut my_parser = parser::Parser::new();
        assert_eq!(my_parser.parse(input).unwrap(), (3*6) as f64);
    }
    #[test]
    fn divide() {
        let input = String::from("15 / 3");
        let mut my_parser = parser::Parser::new();
        assert_eq!(my_parser.parse(input).unwrap(), (15.0/3.0) as f64);
    }
    #[test]
    fn power() {
